...
def normalize_row(row, config):
    ...
    if not timestamp:
        ...
        time_value = get_first(row, ["clock_time", "time_in"])
    ...
    return {
        ...
        "class_name": get_first(row, COLUMN_ALIASES["class_name"]) or config.get("default_class_name", ""),
...
def collect_new_rows(config):
    ...
    if source_was_recreated(csv_path, state):
        state["uploaded_row_count"] = 0

    uploaded_count = int(state.get("uploaded_row_count", 0))
    ...
    if normalized:
        ...
        append_unsent_rows(unsent_path, normalized)

    save_state(state_path, len(all_rows), csv_path.stat().st_size if csv_path.exists() else 0)
